...
prob1(B,C,E,F,H,J) :- 
	threedigitnocarry(0,B,C,0,E,F,0,H,J), 
	all_distinct([B, C, E, F, H, J]).
...
lcoladdcarry(Backcarry, Top, Bottom, Onesresult, Tensresult) :- 
	digit(Backcarry), digit(Top), digit(Bottom),
	Onesresult is mod(Top + Bottom + Backcarry, 10),	
	Tensresult is ((Top + Bottom + Backcarry) - mod(Top + Bottom + Backcarry, 10))/10.
	
fourdigitcarry(A,B,C,D, E,F,G,H, V,W,X,Y,Z) :-  
	rcoladdcarry(D,H,Z,C1), 
	mcoladdcarry(C1,C,G,Y,C2), 
	mcoladdcarry(C2,B,F,X,C3), 
	lcoladdcarry(C3,A,E,W,V). 
